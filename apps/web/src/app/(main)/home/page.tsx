...
function toPlayerTrack(t: FeedTrack): PlayerTrack {
  return {
    ...
    primaryGenre: t.primaryGenre, moods: t.moodsJson ?? [], recommendationReason: t.recommendationReason,
  }
}
...
export default function HomePage() {
  ...
  const { loadQueue, currentTrack, queue, skipNext, interactions, toggleLike, toggleSave, sendDislike } = usePlayerStore()
  ...
  const interaction = interactions[currentTrack.id] ?? { liked: false, saved: false }

  return (
    <>
      {addModalTrackId && (
        <AddToPlaylistModal trackId={addModalTrackId} onClose={() => setAddModalTrackId(null)} />
      )}
      ...
      <div className="flex flex-col h-screen px-6 pt-12 pb-4">
        ...
        <div className="flex-1 flex flex-col justify-center gap-6">
          ...
          <div className="flex items-center justify-center gap-6">
            <ActionBtn icon={ThumbsDown} label="Pass" onClick={() => sendDislike(currentTrack.id)} danger />
            <ActionBtn icon={Heart} label="Like" onClick={() => toggleLike(currentTrack.id)} accent active={interaction.liked} />
            <ActionBtn icon={SkipForward} label="Skip" onClick={() => skipNext()} />
            <ActionBtn icon={Bookmark} label="Save" onClick={() => toggleSave(currentTrack.id)} active={interaction.saved} />
            <ActionBtn icon={Plus} label="Add" onClick={() => setAddModalTrackId(currentTrack.id)} />
          </div>
        </div>
      </div>
    </>
...
function ActionBtn({
  icon: Icon, label, onClick, accent, danger, active,
}: { icon: React.ElementType; label: string; onClick: () => void; accent?: boolean; danger?: boolean; active?: boolean }) {
  return (
    <button
      onClick={onClick}
    ...
    >
      <div className={`w-12 h-12 rounded-full flex items-center justify-center transition-colors ${
        ...
        accent ? 'bg-accent/20 border border-accent/40 text-accent' :
        active ? 'bg-card border border-accent text-accent' :
